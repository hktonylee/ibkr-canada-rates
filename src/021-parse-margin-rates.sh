...
# Script to extract margin loan rate data from HTML and output as CSV
# Expected input: HTML file containing the "Interest Charged on Margin Loans" table
...
# Enter the section once we see the Interest Charged on Margin Loans heading
/Interest Charged on Margin Loans/ { inSection = 1 }

# Start of the target table body
inSection && /<tbody>/ { inTbody = 1; next }

# End of the target table body (stop capturing)
inTbody && /<\/tbody>/ { inTbody = 0; inSection = 0; next }
...
inRow && /<\/tr>/ {
    ...
    # Extract BenchmarkDiff from "(BM +/- X%)" if present; store magnitude
    if (rateTxt != "") {
        tmp = rateTxt
